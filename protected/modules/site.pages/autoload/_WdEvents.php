...
class site_pages_WdEvents
{
	 ...
	 * extend the config block with two elements to set the _view_ and _list_ urls.
	 *
	 * @param WdEvent $ev
	 */

	/* TODO-20100528: this has been deprecated by the new view <-> page relationship method
	 *
	static public function alter_block_config(WdEvent $ev)
	{
		...
		if (!($module instanceof system_nodes_WdModule) || ($module instanceof site_pages_WdModule) || !$core->hasModule('site.pages'))
		{
			return;
		}
		...
		$base = wd_camelCase((string) $module, '.');
				...
				(
					...
					(
						...
						(
							...
							l'affichage d'une entrée. Une URL unique sera crée par la
