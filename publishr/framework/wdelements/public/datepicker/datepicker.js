 ...
 * http://dev.base86.com/solo/51/javascript_datepicker_calender_eightysix_released.html
 *
...
var CalendarEightysix = new Class({
	...
	initialize: function(target, options)
	{
		...
		var value = this.target ? this.target.get('value') : null;

		if(this.options.defaultDate != undefined)
			this.selectedDate = new Date().parse(this.options.defaultDate).clearTime();
		else if(this.options.linkWithInput && value)
			this.selectedDate = new Date().parse(value).clearTime();

		if(this.selectedDate == undefined || !this.selectedDate.isValid())
		...
		if(this.options.excludedDates != undefined) {
			var excludedDates = [];
		...
		*/
		...
		if(this.options.createHiddenInput) {
			this.hiddenInput = new Element('input', { 'type': 'hidden', 'name': this.target.name }).inject(this.target, 'after');
		}
		...
		//Or toggler...
		if(this.options.toggler != undefined) this.options.toggler = $(this.options.toggler);

		//Add show and hide events
		(this.options.toggler ? this.options.toggler : this.target).addEvents
		({
...
//DISABLED		MooTools.lang.addEvent('langChange', function() { this.render(); this.pick(); }.bind(this));
	...
	injectDay: function(row, date, outside)
	{
		...
		if((this.options.minDate != undefined && this.format(this.options.minDate, '%t') > this.format(date, '%t')) ||
		   (this.options.maxDate != undefined && this.format(this.options.maxDate, '%t') < this.format(date, '%t')) ||
		   (this.options.excludedWeekdays != undefined && this.options.excludedWeekdays.contains(date.format('%w').toInt())) ||
		   (this.options.excludedDates != undefined && this.options.excludedDates.contains(this.format(date, '%t'))))
	...
	renderYear: function() {
		...
		for(var i = 1; i < 13; i++) {
			...
			if((this.options.minDate != undefined && this.format(this.options.minDate.clone().set('date', 1), '%t') > this.format(iMonth, '%t')) ||
			   (this.options.maxDate != undefined && this.format(this.options.maxDate.clone().set('date', 1), '%t') < this.format(iMonth, '%t')))
	...
	renderDecade: function()
	{
		...
		for (var i = startYear; i < startYear + 12; i++)
		{
			...
			if ((this.options.minDate != undefined && this.options.minDate.get('year') > i) ||
			   (this.options.maxDate != undefined && this.options.maxDate.get('year') < i))
			{
				year.addClass('c86-non-selectable');
			}
			else
			{
				year.addEvent('click', this.levelDown.bind(this));
			}
	...
	renderAfter: function() {
		...
		for(var i = 0; i < rows.length; i++) {
			...
			if( (this.view == 'month' && i == 1 && this.options.minDate != undefined && this.options.minDate.diff(this.viewDate) <= 0) ||
				(this.view != 'month' && i == 0 && this.options.minDate != undefined && this.format(this.options.minDate, '%t') >= this.format(rows[i].getFirst().retrieve('date'), '%t')) )
				this.arrowLeft.setStyle('visibility', 'hidden');

			if(i == rows.length - 1 && this.options.maxDate != undefined) {
		...
		if(this.$events['render'+ this.view] != undefined) {
	...
	show: function()
	{
		//console.log('SHOW: visible: %d, alwaysShow: %d', this.visible, this.options.alwaysShow);
	...
	pick: function(e) {
		...
		if(this.hiddenInput != undefined) this.hiddenInput.set('value', this.format(this.selectedDate, this.options.hiddenInputFormat));
	...
	correctDate: function(date, right) {
		...
		if(this.options.minDate != undefined && date.diff(this.options.minDate) > 0) date = this.options.minDate.clone();
		//Check if the date is higher than the maximum date
		else if(this.options.maxDate != undefined && date.diff(this.options.maxDate) < 0) date = this.options.maxDate.clone();

		//Check if the currect picked weekday is allowed
		var i = 0;
		while( (undefined != this.options.excludedWeekdays && this.options.excludedWeekdays.contains(date.format('%w').toInt())) ||
			   ( (undefined != this.options.minDate && date.diff(this.options.minDate) > 0) ||
				 (undefined != this.options.maxDate && date.diff(this.options.maxDate) < 0) ) ||
			   (undefined != this.options.excludedDates && this.options.excludedDates.contains(this.format(date, '%t'))) ) {
	...
	{
		if(undefined == pick) pick = true;

		if(typeOf(value) == 'date') {
			var date = value.clearTime();
		} else {
			var date = (!!(value || value === 0)) ? new Date().parse(this.target.get('value')).clearTime() : new Date().clearTime();
		}
