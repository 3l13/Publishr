...
class Textmark_Parser extends MarkdownExtra_Parser
{
	...
	*/
	...
	{
//		wd_log('\1 :: matches: \2', __FUNCTION__, $matches);

		return $this->hashPart($this->doSourceCode($matches[1], $matches[2]), 'B');
	}
	...
	function doSourceCode($type, $text)
	{
		...
		switch ($type)
		{
			case 'php': return $this->doSourcePHP($text);
			case 'html': return $this->doSourceHTML($text);
			case 'raw': return $this->doSourceRaw($text);
			case 'publish': return $this->doSourcePublish($text);
		}

		wd_log_error('\1: unknown source type "\1"', __FUNCTION__, $type);
	...
	*/
	...
	{
//		wd_log('<pre>doHeaders[ext]: \1</pre>', print_r($matches, true));
	...
	{
//		wd_log('<pre>doHeaders[atx]: \1</pre>', print_r($matches, true));
...
//		echo l('doGlyphs: "\1"<br />', wd_entities($text));
	...
	*/
	...
	#
		...
		{
//			wd_log('in \1, graf: [<tt>\2</tt>]', __FUNCTION__, $value);
...
//			wd_log('in \1, graf: <tt>\2</tt>, match: <pre>\3</pre>', __FUNCTION__, $value, $matches);
	...
	*/
	...
	function _doImages_reference_callback($matches)
	{
		if (self::$images_reference_callback === null)
		{
			...
			try
			{
				$hook = WdHook::find('textmark', 'images.reference');
		...
		if (!$module)
		{
			...
			$module = $core->module('resources.images');
		}
...
//		echo l('<pre>in \1: \2</pre>', __FUNCTION__, $matches);
	...
	{
//		wd_log('<pre>in \1: \2</pre>', __FUNCTION__, $matches);
	...
	*/
	...
	function doSourceRaw($text)
	{
		...
		$text = '<code class="html">' . $text . '</code>';
		$text = nl2br($text);
	...
	function doSourceHTML($text)
	{
//		wd_log('## \1 ## <pre>\2</pre>', __FUNCTION__, $text);
...
//		wd_log('## \1 ## <pre>\2</pre>', __FUNCTION__, wd_entities($text));
...
//		preg_match('#\/\*(.*)\*\/#', $text, $matches);
		...
		$this->php_colors = array_flip($colors);

		$text = "<?php\n\n" . trim($text) . "\n\n?>";
		$text = highlight_string($text, true);

//		wd_log('light: <pre>\1</pre>', wd_entities($text));

		// remove default

//		preg_match('#(\<span\sstyle\=\"color\:\s\\' . $colors['default'] . '\"\>([^<]+)\<\/span\>)#', $text, $matches);
...
//		wd_log('light: <pre>\1</pre>', wd_entities($text));
		...
		$text = str_replace('<span style="color: ' . $colors['default'] . '">', '<span class="delimiter">', $text);


//		preg_match('#\?\&gt\;\s*\<\/span\>\s*\<\/code\>#', $text, $matches);

//		wd_log('default matches: <pre>\1</pre>', wd_entities(print_r($matches, true)));

		$text = preg_replace('#\?\&gt\;\s*\<\/span\>\s*\<\/code\>#', '<span class="delimiter">?&gt;</span></code>', $text);

//		preg_match('#(\<span\sstyle\=\"color\:\s\\' .  $colors['default'] . '\"\>)#', $text, $matches);
...
//		wd_log('light: <pre>\1</pre>', wd_entities($text));
		...
		$text = preg_replace('#(\$[^-\[\<]+)#', '<span class="var">\1</span>', $text);

		$text = preg_replace_callback
		(
			'/style\=\"color\:\s(\#[0-9A-F]{6})\"/',
			array(&$this, '_do_php_callback'), $text
		);

		$text = str_replace
		(
			'<code>', '<code class="php">', $text
		);
...
//		wd_log('<pre>in \1: \2</pre>', __FUNCTION__, $text);
...
//					echo l('found marker at \1 (\2)<br />', $i, wd_entities(substr($line, $i, 16)));
	...
	function doSourceString($text)
	{
		...
		for ($i = 0 ; $i < $y ; $i++)
		{
			...
			if (($c == self::QUOTE_SINGLE) || ($c == self::QUOTE_DOUBLE))
			{
				if (($i > 1) && ($text{$i - 1} == '\\'))
				{
					#
					# this is escaped quote
					#
				...
				else if ($quote)
				{
					if ($c == $quote)
					{
						...
						$out = $swap . $this->hashPart('<span class="string">' . wd_entities($out) . '</span>');
						$swap = NULL;
...
//		wd_log('out: <code>\1</code>', nl2br(wd_entities($out)));
...
//		wd_log('## \1 ## <pre>\2</pre>', __FUNCTION__, $text);
		...
		$text = preg_replace_callback
		(
			'#(?<!\')\".*?\"#ms', array(&$this, '_do_php_string'), $text
		);
		...
		*/
		...
		$text = preg_replace_callback
		(
			'#function(\s+[a-zA-z0-9_]+)#', array(&$this, '_do_php_function_def'), $text
		);
		*/
...
//			"\n\n?&gt;</span>" .
	...
	{
//		wd_log('## \1 ## \2', __FUNCTION__, $matches);
