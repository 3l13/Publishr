...
class WdTextHole
{
	...
	{
		/*
		// TODO: move to WdPatron

		$hook = null;

		try
		{
			$hook = WdHook::find('patron.functions', $name);
		}
		catch (Exception $e) { }
...
//			self::set_callback($this->context, array($name => $value));
	...
	static private function set_callback(&$destination, $source)
	{
		...
		if (empty($source))
		{
			//throw new WdException('clear destination: \1', $destination);
			$destination = $source;
		}
		else
		{
...
//						echo l('recurse on key \1<br />', $key);
	...
	*/
			...
			if (($c == ',' || $c == ')') && $function)
			{
				...
				if ($part !== null)
				{
					...
					if (!$quote_closed)
					{
						...
						//wd_log('part: [\1] == [\2]', $part_back, $part);
					}
	...
	protected function parseExpression($expression)
	{
		...
		*/
		...
		if (!$parsed && WDTEXTHOLE_USE_APC)
		{
			...
			//wd_log("expression: <em>$expression</em> from APC");
	...
	public function evaluate($expression, $silent=false)
	{
		...
		if (is_array($silent))
		{
			WdDebug::trigger('<em>silent</em> should be a boolean, options are deprecated: \1', array($silent));
		}
		...
		if ($expression{0} == '@')
		{
			...
			if (!array_key_exists('this', $this->context))
			{
				$this->error
				(
					'Using instance variable when no instance is defined: %identifier', array
					(
			...
			switch ($part[self::TOKEN_TYPE])
			{
				...
				case self::TOKEN_TYPE_FUNCTION:
				{
					...
					if (!$callback)
					{
						...
						{
							if (function_exists('wd_array_' . $method))
							{
								$callback = 'wd_array_' . $method;
							}
							else if (method_exists('WdArray', $method))
							{
								$callback = array('WdArray', $method);
							}
