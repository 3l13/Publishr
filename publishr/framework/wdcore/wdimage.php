...
class WdImage
{
	...
	*/
	...
	public function __construct($file=null)
	{
		...
		if (!$info)
		{
			throw new WdException('Unable to get information from %file', array('%file' => $file));
		}
		...
		switch ($this->mime)
		{
			...
			default:
			{
				throw new WdException('Unsupported image type: %mime', array('%mime' => $this->mime));
			}
			break;
		}
		...
		if (empty($this->image))
		{
			throw new WdException('Unable to create image from %file', array('%file' => $file));
		}
...
//		wd_log('image create for \1 : \2', $file, $this->image);
	...
	*/
	...
	static public function load($file, &$info)
	{
		...
		if (!$image)
		{
			WdDebug::trigger('Unable to create image from %file', array('%file' => $file));
	...
	public function save($file, $quality=75)
	{
		...
		switch($ext)
		{
			...
			case "png":
			{
				$done = imagepng($this->image, $file, 9);
			}
			break;

			case "gif":
			{
				$done = imagegif($this->image, $file);
			}
			break;

			default:
			{
				WdDebug::trigger('Unknown extention %extension for %file', array('%extension' => $ext, '%file' => $file));
			}
			break;
		}
	...
	*/
	...
	static public function drawGrid($image, $x1, $y1, $x2, $y2, $color1=0xFFFFFF, $color2=0xCCCCCC, $size=4)
	{
		#
		# resolve size
		#
	...
	static public function decodeColor($color)
	{
		...
		if ($len >= 4)
		{
			...
			// FIXME-20090418: add support for rgb()
		}
