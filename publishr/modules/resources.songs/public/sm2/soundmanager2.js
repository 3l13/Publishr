...
function SoundManager(smURL, smID)
{
	...
	this.flashVersion = 8; // version of flash to require, either 8 or 9. Some
							// API features require Flash 9.
	this.debugMode = true; // enable debugging output (div#soundmanager-debug,
							// OR console if available+configured)
	this.debugFlash = false; // enable debugging output inside SWF,
								// troubleshoot Flash/browser issues
	this.useConsole = true; // use firebug/safari console.log()-type debug
							// console if available
	this.consoleOnly = false; // if console is being used, do not create/write
								// to #soundmanager-debug
	this.waitForWindowLoad = true; // force SM2 to wait for window.onload()
									// before trying to call
									// soundManager.onload()
	this.nullURL = 'null.mp3'; // path to "null" (empty) MP3 file, used to
								// unload sounds (Flash 8 only)
	this.allowPolling = true; // allow flash to poll for status update
								// (required for whileplaying() events, peak,
								// sound spectrum functions to work.)
	this.useFastPolling = false; // uses 1 msec flash timer interval (vs.
									// default of 20) for higher callback
									// frequency, best combined with
									// useHighPerformance
	this.useMovieStar = false; // enable support for Flash 9.0r115+ (codename
								// "MovieStar") MPEG4 audio+video formats (AAC,
								// M4V, FLV, MOV etc.)
	this.bgColor = '#ffffff'; // movie (.swf) background color, '#000000'
								// useful if showing on-screen/full-screen video
								// etc.
	this.useHighPerformance = false; // position:fixed flash movie can help
										// increase js/flash speed, minimize lag
	this.flashLoadTimeout = 1000; // msec to wait for flash movie to load
									// before failing (0 = infinity)
	this.wmode = null; // mode to render the flash movie in - null,
						// transparent, opaque (last two allow layering of HTML
						// on top)
	this.allowFullScreen = true; // enter full-screen (via double-click on
									// movie) for flash 9+ video
	this.allowScriptAccess = 'always'; // for scripting the SWF (object/embed
										// property), either 'always' or
										// 'sameDomain'

	this.defaultOptions = {
	'autoLoad' : false, // enable automatic loading (otherwise .load() will be
						// called on demand with .play(), the latter being nicer
						// on bandwidth - if you want to .load yourself, you
						// also can)
	'stream' : true, // allows playing before entire file has loaded
						// (recommended)
	'autoPlay' : false, // enable playing of file as soon as possible (much
						// faster if "stream" is true)
	'onid3' : null, // callback function for "ID3 data is added/available"
	'onload' : null, // callback function for "load finished"
	'whileloading' : null, // callback function for "download progress update"
							// (X of Y bytes received)
	...
	'onbeforefinish' : null, // callback for "before sound finished playing
								// (at [time])"
	'onbeforefinishtime' : 5000, // offset (milliseconds) before end of sound
									// to trigger beforefinish (eg. 1000 msec =
									// 1 second)
	'onbeforefinishcomplete' : null, // function to call when said sound
										// finishes playing
	'onjustbeforefinish' : null, // callback for [n] msec before end of
									// current sound
	'onjustbeforefinishtime' : 200, // [n] - if not using, set to 0 (or null
									// handler) and event will not fire.
	'multiShot' : true, // let sounds "restart" or layer on top of each other
						// when played multiple times, rather than one-shot/one
						// at a time
	'multiShotEvents' : false, // fire multiple sound events (currently
								// onfinish() only) when multiShot is enabled
	'position' : null, // offset (milliseconds) to seek to within loaded sound
						// data.
	...
	this.flash9Options = { // flash 9-only options, merged into defaultOptions
							// if flash 9 is being used
	'isMovieStar' : null, // "MovieStar" MPEG4 audio/video mode. Null
							// (default) = auto detect MP4, AAC etc. based on
							// URL. true = force on, ignore URL
	'usePeakData' : false, // enable left/right channel peak (level) data
	'useWaveformData' : false, // enable sound spectrum (raw waveform data) -
								// WARNING: CPU-INTENSIVE: may set CPUs on fire.
	'useEQData' : false, // enable sound EQ (frequency spectrum data) -
							// WARNING: Also CPU-intensive.
	'onbufferchange' : null, // callback for "isBuffering" property change
	'ondataerror' : null
	// callback for waveform/eq data access error (flash playing audio in other
	// tabs/domains)
	};

	this.movieStarOptions = { // flash 9.0r115+ MPEG4 audio/video options,
								// merged into defaultOptions if flash
								// 9+movieStar mode is enabled
	'onmetadata' : null, // callback for when video width/height etc. are
							// received
	'useVideo' : false, // if loading movieStar content, whether to show video
	'bufferTime' : null
	// seconds of data to buffer before playback begins (null = flash default of
	// 0.1 seconds - if AAC playback is gappy, try up to 3 seconds)
	};

	// jslint global declarations
	/*
	 * global SM2_DEFER, sm2Debugger, alert, console, document, navigator,
	 * setTimeout, window
	 */
	...
	this._str = function()
	{ // o [,items to replace]
		var params = Array.prototype.slice.call(arguments); // real array,
															// please
	...
	this.createSound = function(oOptions)
	{
		...
		{ // hopefully this isn't buggy regexp-fu. :D
		...
		{
			_s.o
			        ._createSound(_tO.id, _tO.url, _tO.onjustbeforefinishtime, _tO.usePeakData, _tO.useWaveformData, _tO.useEQData, _tO.isMovieStar, (_tO.isMovieStar ? _tO.useVideo : false), (_tO.isMovieStar ? _tO.bufferTime : false));
		}
	...
	this.destroySound = function(sID, bFromSound)
	{
		...
		// calling destroySound() within a sound onload() might crash firefox,
		// certain flavours of winXP+flash 8??
		// if (_s.flashVersion != 8) {
		...
		if (!_s._idCheck(sID))
		{
			if (typeof oOptions != 'Object')
			{
				oOptions = {
					...
					// play('mySound','/path/to/some.mp3');
			}
			if (oOptions && oOptions.url)
			{
				// overloading use case, creation+playing of sound:
				// .play('someID',{url:'/path/to.mp3'});
	...
	this.onready = function(oMethod, oScope)
	{
		...
		/*
		 * soundManager.onready(function(oStatus) { alert('SM2 init success:
		 * '+oStatus.success); });
		 */
	...
	{
		// called after SWF has been appended to the DOM via JS (or retrieved
		// from HTML)
	...
	{
		// Try to create meaningful custom errors, w/stack trace to the
		// "offending" line
		var sPre = 'Error: ';
		if (!oCaller)
		{
		...
		if (e.stack)
		{
			// potentially dangerous: Try to return a meaningful stacktrace
			// where provided (Mozilla)
			try
			{
				var splitChar = '@';
				var stackTmp = e.stack.split(splitChar);
				stackMsg = stackTmp[4]; // try to return only the relevant bit,
										// skipping internal SM2 shiz
			} catch (ee)
			{
				// oops.
				stackMsg = e.stack;
		...
		// See JS error/debug/console output for real error source, stack trace
		// / message detail where possible.
	...
	{
		return (document.body ? document.body : (document.documentElement ? document.documentElement : document
	...
	this._createMovie = function(smID, smURL)
	{
		...
		if (_s.wmode !== null && _s.flashLoadTimeout !== 0 && (!_s.useHighPerformance || _s.debugFlash) && !_s.isIE && navigator.platform
		        .match(/win32/i))
		{
			_s.specialWmodeCase = true;
			// extra-special case: movie doesn't load until scrolled into view
			// when using wmode = anything but 'window' here
			// does not apply when using high performance (position:fixed means
			// on-screen), OR infinite flash load timeout
			_s._wDS('spcWmode');
			_s.wmode = null;
		}
		...
		if (oTarget)
		{
			...
			if (!_s.oMC.id)
			{
				...
				if (_s.useHighPerformance)
				{
					...
					left : '0px',
					overflow : 'hidden'
					// zIndex:-1 // sit behind everything else - potentially
					// dangerous/buggy?
					};
				} else
				{
		...
		_s
		        ._wD('-- SoundManager 2 ' + _s.version + (_s.useMovieStar ? ', MovieStar mode' : '') + (_s.useHighPerformance ? ', high performance mode, ' : ', ') + ((_s.useFastPolling ? 'fast' : 'normal') + ' polling') + (_s.wmode ? ', wmode: ' + _s.wmode : '') + (_s.debugFlash ? ', flash debug mode' : '') + ' --', 1);
		_s
		        ._wD('soundManager._createMovie(): Trying to load ' + smURL + (!_s._overHTTP && _s.altURL ? ' (alternate URL)' : ''), 1);
	};
	...
	this._initMovie = function()
	{
		...
		if (_s.o)
		{
			// _s._wD('soundManager._initMovie(): Got '+_s.o.nodeName+' element
			// ('+(_s._didAppend?'created via JS':'static HTML')+')',1);
	...
	this.waitForExternalInterface = function()
	{
		...
		if (_s.flashLoadTimeout > 0)
		{
			if (!_s._didInit)
			{
				...
				_s
				        ._wD(_s
				                ._str('waitImpatient', (p == 100 ? ' (SWF loaded)' : (p > 0 ? ' (SWF ' + p + '% loaded)' : ''))));
			}
			setTimeout(function()
			{
				var p = _s.getMoviePercent();
				...
				{
					_s
					        ._wD(_sm + ': No Flash response within reasonable time after document load.\nLikely causes: ' + (p === null || p === 0 ? 'Loading ' + _s.movieURL + ' may have failed (and/or Flash ' + _s.flashVersion + '+ not present?), ' : '') + 'Flash blocked or JS-Flash security error.' + (_s.debugFlash ? ' ' + _s
					                ._str('checkSWF') : ''), 2);
					if (!_s._overHTTP)
					{
						_s._wDS('localFail', 2);
	...
	this.handleFocus = function()
	{
		...
		{
			// giant Safari 3.1 hack - assume window in focus if mouse is
			// moving, since document.hasFocus() not currently implemented.
	...
	this.initComplete = function(bNoDisable)
	{
		...
		_s._didInit = true;
		_s
		        ._wD('-- SoundManager 2 ' + (_s._disabled ? 'failed to load' : 'loaded') + ' (' + (_s._disabled ? 'security/load error' : 'OK') + ') --', 1);
		if (_s._disabled || bNoDisable)
		{
			// _s._wD('soundManager.initComplete(): calling
			// soundManager.onerror()',1);
			_s._processOnReady();
	...
	this._processOnReady = function()
	{
		...
		{
			_s
			        ._wD(_sm + ': Firing ' + queue.length + ' onready() item' + (queue.length > 1 ? 's' : ''));
	...
	{
		// callback from flash for confirming that movie loaded, EI is working
		// etc.
		...
		_s
		        ._wD('soundManager._externalInterfaceOK()' + (flashDate ? ' (~' + (eiTime - flashDate) + ' ms)' : ''));
		...
		if (_s.isIE)
		{
			// IE needs a timeout OR delay until window.onload - may need TODO:
			// investigating
	...
	this._setSandboxType = function(sandboxType)
	{
		...
		if (sb.type == 'localWithFile')
		{
			...
			if (sb.type == 'localWithNetwork')
			{
				sb.noRemote = false;
				sb.noLocal = true;
			} else
				if (sb.type == 'localTrusted')
				{
					sb.noRemote = false;
					sb.noLocal = false;
				}
	...
	SMSound = function(oOptions)
	{
		...
		this.instanceOptions = this.options; // per-play-instance-specific
												// options
		...
		this._debug = function()
		{
			if (_s.debugMode)
			{
				...
				var sF = null;
				var sfBracket = null;
				var maxLength = 64; // # of characters of function code to show
									// before truncating
				for (stuff in _t.options)
				{
					if (_t.options[stuff] !== null)
					{
						if (_t.options[stuff] instanceof Function)
						{
							...
							msg[msg.length] = ' ' + stuff + ': {' + sF.substr(sfBracket + 1, (Math
							        .min(Math.max(sF.indexOf('\n') - 1, maxLength), maxLength)))
							        .replace(/\n/g, '') + '... }';
						} else
						{
		 ...
		 * names (download documentation):
		 * http://livedocs.macromedia.com/flash/8/ Previously-live URL:
		...
		this.resetProperties = function(bLoaded)
		{
			...
			this.playState = 0;
			this.paused = false;
			this.readyState = 0; // 0 = uninitialised, 1 = loading, 2 =
								// failed/error, 3 = loaded/success
			this.muted = false;
			this.didBeforeFinish = false;
			...
			this.eqData = [];
			// dirty hack for now: also have left/right arrays off this,
		...
		this.load = function(oOptions)
		{
			...
			_t.loaded = false;
			_t.readyState = 1;
			_t.playState = 0; // (oOptions.autoPlay?1:0); // if autoPlay,
								// assume "playing" is true (no way to detect
								// when it actually starts in Flash unless
								// onPlay is watched?)
			try
			{
				if (_s.flashVersion == 8)
				...
				else
				{
					...
					{
						// special case: MPEG4 content must start playing to
						// load, then pause to prevent playing.
						_t.pause();
					}
				}
			} catch (e)
		...
		this.unload = function()
		{
			...
			if (_t.readyState !== 0)
			{
				...
				if (_t.readyState != 2)
				{ // reset if not error
					_t.setPosition(0, true); // reset current sound
												// positioning
				}
		...
		this.play = function(oOptions)
		{
			...
			if (!_t.loaded)
			{
				if (_t.readyState === 0)
				{
					...
					// if (typeof oOptions.autoPlay=='undefined') _tO.autoPlay =
					// true; // only set autoPlay if unspecified here
			...
			} else
			{
				...
				_t.setVolume(_t._iO.volume, true); // restrict volume to
													// instance options only
				_t.setPan(_t._iO.pan, true);
				_s.o
				        ._start(_t.sID, _t._iO.loop || 1, (_s.flashVersion == 9 ? _t.position : _t.position / 1000));
			}
		};
		...
		this.stop = function(bAll)
		{
			if (_t.playState == 1)
			{
				_t.playState = 0;
				_t.paused = false;
				// if (_s.defaultOptions.onstop)
				// _s.defaultOptions.onstop.apply(_s);
				if (_t._iO.onstop)
				{
		...
		this.setPosition = function(nMsecOffset, bNoDebug)
		{
			...
			var offset = Math.min(_t.duration, Math.max(nMsecOffset, 0)); // position
																			// >= 0
																			// and
																			// <=
																			// current
			...
			if (!bNoDebug)
			{
				// _s._wD('SMSound.setPosition('+nMsecOffset+')'+(nMsecOffset !=
				// offset?', corrected value: '+offset:''));
			}
			_s.o
			        ._setPosition(_t.sID, (_s.flashVersion == 9 ? _t._iO.position : _t._iO.position / 1000), (_t.paused || !_t.playState)); // if
																																			// paused
																																			// or
																																			// not
																																			// playing,
		...
		this.resume = function()
		{
			...
			_t.paused = false;
			_s.o._pause(_t.sID); // flash method is toggle-based
									// (pause/resume)
			if (_t._iO.onresume)
			{
		...
		this.togglePause = function()
		{
			...
			{
				this.play( {
					position : (_s.flashVersion == 9 ? this.position : this.position / 1000)
				});
		...
		this.setVolume = function(nVol, bInstanceOnly)
		{
			...
			_s.o._setVolume(this.sID, (_s.muted && !this.muted) || this.muted ? 0 : nVol);
			this._iO.volume = nVol;
		...
		{
			this.bytesLoaded = nBytesLoaded;
			this.bytesTotal = nBytesTotal;
			this.duration = Math.floor(nDuration);
			this.durationEstimate = this.duration;

			if (!this._iO.isMovieStar)
			{
				var estimate = parseInt((this.bytesTotal / this.bytesLoaded) * this.duration, 10);

				if (estimate !== undefined)
				{
					this.durationEstimate = estimate;
				}
				/*
				 * else { // reported bug? }
				 */
			...
			if (this.readyState != 3 && this._iO.whileloading)
			{
				this._iO.whileloading.apply(this);
			}
		};
		...
		this._whileplaying = function(nPosition, oPeakData, oWaveformDataLeft, oWaveformDataRight, oEQData)
		{
			...
			if (this.playState === 0 && nPosition > 0)
			{
				// can happen at the end of a video where nPosition == 33 for
				// some reason, after finishing.???
				// can also happen with a normal stop operation. This resets the
				// position to 0.
				// _s._writeDebug('Note: Not playing, but position =
				// '+nPosition);
			...
			if (_s.flashVersion > 8)
			{
				if (this._iO.usePeakData && typeof oPeakData != 'undefined' && oPeakData)
				{
					this.peakData = {
				...
				if (this._iO.useWaveformData && typeof oWaveformDataLeft != 'undefined' && oWaveformDataLeft)
				{
					this.waveformData = {
				...
				if (this._iO.useEQData)
				{
					if (typeof oEQData != 'undefined' && oEQData.leftEQ)
					{
						...
						this.eqData = eqLeft;
						this.eqData.left = eqLeft;

						if (typeof oEQData.rightEQ != 'undefined' && oEQData.rightEQ)
						{
							this.eqData.right = oEQData.rightEQ.split(',');
						}
					}
				}
			}
			...
			if (this.playState == 1)
			{
				// special case/hack: ensure buffering is false (instant load
				// from cache, thus buffering stuck at 1?)
				...
				if (this._iO.whileplaying)
				{
					this._iO.whileplaying.apply(this); // flash may call after
														// actual finish
				}

				if (this.loaded && this._iO.onbeforefinish && this._iO.onbeforefinishtime && !this.didBeforeFinish && this.duration - this.position <= this._iO.onbeforefinishtime)
				{
					_s
					        ._wD('duration-position &lt;= onbeforefinishtime: ' + this.duration + ' - ' + this.position + ' &lt= ' + this._iO.onbeforefinishtime + ' (' + (this.duration - this.position) + ')');
		...
		this._onload = function(bSuccess)
		{
			...
			_s
			        ._wD(fN + '"' + _t.sID + '"' + (bSuccess ? ' loaded.' : ' failed to load? - ' + _t.url), (bSuccess ? 1 : 2));
			if (!bSuccess)
			{
		...
		this._onfinish = function()
		{
			...
			// TODO: calling user-defined onfinish() should happen after
			// setPosition(0)
		...
		this._onmetadata = function(oMetaData)
		{
			...
			_s._wD(fN);
			// Contains a subset of metadata. Note that files may have their own
			// unique metadata.
		...
		this._ondataerror = function(sError)
		{
			...
			if (_t.playState > 0)
			{ // hack: called at start, and end from flash at/after
				// onfinish().
	...
	if (window.addEventListener)
	{
		...
		if (_s._tryInitOnFocus)
		{
			window.addEventListener('mousemove', _s.handleFocus, false); // massive
																			// Safari
																			// focus
																			// hack
		}
		...
		{
			// no add/attachevent support - safe to assume no JS -> Flash
			// either.
			_s._debugTS('onload', false);
...
// var SM2_DEFER = true; // un-comment or define in your own script to prevent
// immediate SoundManager() constructor call+start-up.
// if deferring, construct later with window.soundManager = new SoundManager();
// followed by soundManager.beginDelayedInit();
